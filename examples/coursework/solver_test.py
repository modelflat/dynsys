...
def call_solve_equations(a, b, c, root=-1):
    assert len(a) == len(b) == len(c)
    n = len(a)
    a = numpy.array(a, dtype=numpy.complex128)
    a_dev = copy_dev(ctx, a.view(numpy.float64))
    b = numpy.array(b, dtype=numpy.complex128)
    b_dev = copy_dev(ctx, b.view(numpy.float64))
    c = numpy.array(c, dtype=numpy.complex128)
...
def test_single(a: complex, b: complex, c: complex):
    res, err = call_solve_equations([a], [b], [c])
    res = res.flatten()
