...
class Task1(SimpleApp):
    ...
    def __init__(self):
        ...
        self.attr = PhasePortrait(self.ctx, self.queue, self.w, self.h, self.bounds, system_function_source, type_config=double_config)
        self.attr_image = ParametrizedImageWidget(self.bounds)

        self.lam = ObservableValue.makeAndConnect(0, self.draw_attr)
        self.k = ObservableValue.makeAndConnect(1, self.draw_attr)

        self.lam_slider = RealSlider.makeAndConnect(0, 4, self.lam.value(), connect_to=self.lam.setValue)
        self.k_slider = RealSlider.makeAndConnect(1, 4, self.k.value(), connect_to=self.k.setValue)
